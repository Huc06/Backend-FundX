...
export class SupabaseService implements IDatabaseService, OnModuleInit {
  ...
  async createCampaignWithDetails(campaign: any): Promise<any> {
    const { data, error } = await this.client.rpc('create_campaign_with_details', {
    ...
    if (error) {
      this.logger.error('Error calling create_campaign_with_details RPC', error);
  ...
  async createCampaign(campaign: any): Promise<any> {
    const { data, error } = await this.client.from('campaigns').insert([campaign]).select();
    if (error) {
  ...
  async getCampaignByBlobId(blobId: string): Promise<any | null> {
    ...
    const { data, error } = await this.client.from('campaigns').select('*').eq('id', blobId).single();
    if (error) {
      this.logger.error(error);
  ...
  async getCampaignByObjectId(objectId: string): Promise<any | null> {
    const { data, error } = await this.client.from('campaigns').select('*').eq('on_chain_object_id', objectId).single();
    if (error) {
        this.logger.error(error);
        if (error.code === 'PGRST116') return null;
        throw new Error(error.message);
  ...
  async getCampaignsByCreator(creatorAddress: string): Promise<any[]> {
    ...
    const { data: user, error: userError } = await this.client.from('users').select('id').eq('wallet_address', creatorAddress).single();
    if(userError) {
    ...
    const { data, error } = await this.client.from('campaigns').select('*').eq('creator_id', user.id);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async createImage(image: any): Promise<any> {
    const { data, error } = await this.client.from('campaign_gallery_images').insert([image]).select();
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getImagesByCampaignId(campaignId: string): Promise<any[]> {
    const { data, error } = await this.client.from('campaign_gallery_images').select('*').eq('campaign_id', campaignId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getStorySectionsByCampaignId(campaignId: string): Promise<any[]> {
    const { data, error } = await this.client.from('campaign_story_sections').select('*').eq('campaign_id', campaignId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getRoadmapPhasesByCampaignId(campaignId: string): Promise<any[]> {
    const { data, error } = await this.client.from('campaign_roadmap_phases').select('*').eq('campaign_id', campaignId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getTeamMembersByCampaignId(campaignId: string): Promise<any[]> {
    const { data, error } = await this.client.from('campaign_team_members').select('*').eq('campaign_id', campaignId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async createMilestone(milestone: any): Promise<any> {
    const { data, error } = await this.client.from('milestones').insert([milestone]).select();
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getMilestonesByObjectId(objectId: string): Promise<any[]> {
    ...
    const { data: campaign, error: campaignError } = await this.client.from('campaigns').select('id').eq('on_chain_object_id', objectId).single();
    if(campaignError) {
    ...
    const { data, error } = await this.client.from('milestones').select('*').eq('campaign_id', campaign.id);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getMilestone(campaignId: string, milestoneId: string): Promise<any | null> {
    const { data, error } = await this.client.from('milestones').select('*').eq('campaign_id', campaignId).eq('id', milestoneId).single();
    if (error) {
        this.logger.error(error);
        if (error.code === 'PGRST116') return null;
        throw new Error(error.message);
  ...
  async updateMilestoneVoteResult(campaignId: string, milestoneId: string, voteResult: number): Promise<void> {
    const { error } = await this.client.from('milestones').update({ vote_result: voteResult }).eq('campaign_id', campaignId).eq('id', milestoneId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async updateMilestoneIsClaimed(campaignId: string, milestoneId: string, isClaimed: boolean): Promise<void> {
    const { error } = await this.client.from('milestones').update({ is_claimed: isClaimed }).eq('campaign_id', campaignId).eq('id', milestoneId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getMilestonesByCampaignIdAndStatus(campaignId: string, status: string): Promise<any[]> {
    const { data, error } = await this.client.from('milestones').select('*').eq('campaign_id', campaignId).eq('status', status);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async createContribution(contribution: any): Promise<any> {
    const { data, error } = await this.client.from('contributions').insert([contribution]).select();
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getContributionsByAddress(address: string): Promise<any[]> {
    ...
    const { data: user, error: userError } = await this.client.from('users').select('id').eq('wallet_address', address).single();
    if(userError) {
        this.logger.error(userError);
        if (userError.code === 'PGRST116') return [];
        throw new Error(userError.message);
    ...
    const { data, error } = await this.client.from('contributions').select('*').eq('user_id', user.id);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getContributionsByCampaignId(campaignId: string): Promise<any[]> {
    const { data, error } = await this.client.from('contributions').select('*').eq('campaign_id', campaignId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getEventMilestonesByEventId(eventId: string): Promise<any[]> {
    const { data, error } = await this.client.from('milestones').select('*').eq('event_id', eventId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getEventServicesByEventId(eventId: string): Promise<any[]> {
    const { data, error } = await this.client.from('event_services').select('*').eq('event_id', eventId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
  ...
  async getEventGalleryImagesByEventId(eventId: string): Promise<any[]> {
    const { data, error } = await this.client.from('event_gallery_images').select('*').eq('event_id', eventId);
    if (error) {
        this.logger.error(error);
        throw new Error(error.message);
    }
